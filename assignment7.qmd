...
Calculating means with `summarise` grouped by species and traits.
...
Then filtering to leave out unwanted species. 
Finally finding the maximums from groups of specific combinations of Spiecies, Part and Measure.
...
```
```{r}
#joining the tables by Spiecies, Part and Measure
iris_joined <- iris_long_sep %>%
  left_join(y = iris_mean_group, by = c("Species", "Part", "Measure")) %>%
  #filtering according to comparisons od Size and means
